...
#'
#' @param path character; default is "data/ABS". Only used if retain_files is set to
...
read_cpi <- function(path = "data/ABS",
  ...
  if(retain_files == TRUE & !is.character(path)){
    stop("If `retain_files` is `TRUE`, you must specify a valid file path in `path`.")
  }
  ...
  x <- cpi_raw %>%
    dplyr::filter(series == "Index Numbers ;  All groups CPI ;  Australia ;") %>%
    dplyr::select(date,
                  cpi = value)
