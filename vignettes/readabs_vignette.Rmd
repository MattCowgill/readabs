---
title: "Working with ABS time series data"
output: rmarkdown::html_vignette
author: "Matt Cowgill"
vignette: >
  %\VignetteIndexEntry{Working with ABS time series data}
...
Each of these steps can be a pain. `{readabs}`  provides functions to help you with each of those steps. One key function -- `readabs::read_abs()` -- performs all three steps if you're working with time series data.

If the data you want to load is not stored in a standard ABS time series format, 
`{readabs}` has other functions that can help. See the [vignette on downloading data cubes](read_data_cubes.html) for more on this.
...
A benefit of this approach is that the filtering is done for you - the data frame only contains the series you're interested in. You can also supply a vector of multiple series IDs. 

## {readabs} only works with time series

The spreadsheets on the ABS website are divided into one of two categories: **time series spreadsheets** and **data cubes**. Many ABS releases include spreadsheets of both types, though it's harder to tell them apart on the new ABS website.
...
`read_abs()` can download and tidy data contained in ABS time series spreadsheets. 

The `download_abs_data_cube()` function can download data cubes. See [the vignette on data cubes](read_data_cubes.html) for more on this and related functions.

## Where do the spreadsheets go?
...
If you run `read_abs_local()` without any arguments specified, it will look in the directory given by the `R_READABS_PATH` environment variable (if set) and attempt to read any .xls files located there (note: it won't look in any of its subdirectories).
...
If you want to read a particular table, or tables, you can specify them using the `filenames` argument. If your files are not in `Sys.getenv("R_READABS_PATH")` or if they are in a subdirectory of this path, you'll need to specify the directory using `path`, like this:
...
See [the vignette on data cubes](read_data_cubes.html) for an overview of some other functions in `{readabs}`.
