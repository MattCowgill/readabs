...
Each of these steps can be a pain. The {readabs} package provides functions to help you with each of those steps. One key function -- `read_abs()` -- performs all three steps if you're working with time series data.
...
Another function, `read_abs_local()` is useful if you have already downloaded ABS time series spreadsheet to disk; it imports and tidies the spreadsheets. More information is below.
...
The `read_abs()` function does a lot of the work for you by getting your data in a tidy (long) format. This makes it easier to filter to the data you're interested in and get to your analysis. But you'll often want to do some further cleaning up of the data to make it easier to work with. For example, the `series` column of a table imported using `read_abs()` often contains a long, multi-part string. This is where the `separate_series()` function comes in handy.
...
In this vignette, the location of the files we've downloaded so far depend on whether you have this environment variable set. To demonstrate `read_abs_local()` we'll need to know the path in order to demonstrate how to use `read_abs_local()`
